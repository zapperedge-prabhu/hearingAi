/**
 * Content Understanding (CU) Result Persistence Service
 * 
 * Handles storage of CU analysis results in Azure Blob Storage.
 * 
 * Storage Structure:
 * /cu_folder/{original_directory}/{filename}_cu_result_{timestamp}_{number}.json
 * 
 ...
 * - cu_results_list: comma-separated result numbers (e.g., "1,2,3")
 * - cu_result_{n}_path: blob path to CU result file
 * - cu_result_{n}_timestamp: when it was saved
 * - cu_result_{n}_analyzedby: user email who analyzed
 * 
 * Example:
 * Original file: documents/reports/quarterly.pdf
 * CU result: cu_folder/documents/reports/quarterly.pdf_cu_result_20250110_143052_001.json
 * Source blob metadata: { cu_results_list: "1", cu_result_1_path: "cu_folder/...", ... }
 * 
 * Key Rules:
...
// Configurable CU results directory via environment variable (default: cu_folder)
const CU_FOLDER_NAME = process.env.ZAPPER_CU_RESULTS_DIR || "cu_folder";
// No hard limit on results per file — unlimited saves are allowed
const MAX_RESULTS_PER_FILE = parseInt(process.env.ZAPPER_CU_MAX_RESULTS_PER_FILE || "0", 10);
const CU_RESULT_PREFIX = "cu_result_";

export interface SaveCuResultRequest {
...
export interface SaveCuResultResponse {
  success: boolean;
  blobPath?: string;
  resultNumber?: number;
  error?: string;
...
export interface ListCuResultsResponse {
  success: boolean;
  results?: CuResultItem[];
  count?: number;
  error?: string;
}

export interface GetCuResultResponse {
  success: boolean;
  result?: any;
  metadata?: Record<string, string>;
  error?: string;
...
export interface DeleteCuResultResponse {
  success: boolean;
  message?: string;
  error?: string;
}
 ...
 * Follows AI Agent/Decrypt naming convention: cu_folder/{original_directory}
 * 
 * Example:
 * Source: documents/reports/quarterly.pdf
 * CU Folder: cu_folder/documents/reports
 */
function getCuFolderPath(sourceFilePath: string): string {
  const dirPath = getDirectoryPath(sourceFilePath);
  return dirPath ? `${CU_FOLDER_NAME}/${dirPath}` : CU_FOLDER_NAME;
}

/**
 * Generates the CU result file name.
 * Format: {filename.ext}_cu_result_{timestamp}_{number}.json
 * 
 * Example: quarterly.pdf_cu_result_20250110_143052_001.json
 * 
...
function generateCuResultFileName(sourceFilePath: string, resultNumber: number): string {
  ...
  return `${sanitizedFileName}_cu_result_${timestamp}_${formatResultNumber(resultNumber)}.json`;
}

/**
 * Parses result number from blob name.
 * New format: filename_cu_result_20250110_143052_003.json -> 3
 * Old format: cu_result_003.json -> 3
 */
function parseResultNumber(blobName: string): number {
  // New format: {filename}_cu_result_{timestamp}_{number}.json
  const newMatch = blobName.match(/_cu_result_\d{8}_\d{6}_(\d{3})\.json$/);
  if (newMatch) {
    return parseInt(newMatch[1], 10);
  }
  // Legacy format: cu_result_003.json
  const oldMatch = blobName.match(/cu_result_(\d+)\.json$/);
...
function isCuResultForFile(blobName: string, sourceFileName: string): boolean {
  ...
  // Check if blob name starts with the full source file name and contains _cu_result_
  // Pattern: {filename.ext}_cu_result_{timestamp}_{number}.json
  const pattern = new RegExp(`^${escapeRegExp(sanitizedSourceFileName)}_cu_result_\\d{8}_\\d{6}_\\d{3}\\.json$`, 'i');
 ...
 */
export class CuPersistenceService {
  ...
  async verifySourceFileExists(
    ...
    try {
      ...
      console.log(`[CU-PERSIST] Verifying file exists: ${sourceFilePath} -> decoded: ${decodedPath}`);
      const blobClient = containerClient.getBlobClient(decodedPath);
      return await blobClient.exists();
    } catch (error) {
      console.error(`[CU-PERSIST] Error verifying source file:`, error);
      return false;
    }
  }
  ...
  async getCuResultsMetadata(
    ...
    try {
      ...
      // Parse cu_results_list: comma-separated result numbers
      const resultNumbersStr = metadata.cu_results_list || '';
      ...
      for (const num of resultNumbers) {
        const pathKey = `cu_result_${num}_path`;
        const timestampKey = `cu_result_${num}_timestamp`;
        const analyzedByKey = `cu_result_${num}_analyzedby`;
      ...
      console.log(`[CU-PERSIST] Retrieved CU metadata for ${sourceFilePath}: ${resultNumbers.length} result(s)`);
      
      return { resultNumbers, results };
    } catch (error: any) {
      // If blob doesn't exist or no metadata, return empty
      if (error.statusCode === 404) {
        console.log(`[CU-PERSIST] Source blob not found: ${sourceFilePath}`);
        return { resultNumbers: [], results: {} };
      }
      console.error(`[CU-PERSIST] Error getting CU metadata:`, error.message);
      return { resultNumbers: [], results: {} };
    }
  }
  ...
  async setCuResultMetadata(
    ...
    try {
      ...
      const existingResultsStr = existingMetadata.cu_results_list || '';
      ...
      const newMetadata = {
        ...existingMetadata,
        cu_results_list: existingNumbers.join(','),
        [`cu_result_${resultNumber}_path`]: cuResultBlobPath,
        [`cu_result_${resultNumber}_timestamp`]: new Date().toISOString(),
        [`cu_result_${resultNumber}_analyzedby`]: analyzedBy,
      };
      
      await blobClient.setMetadata(newMetadata);
      
      console.log(`[CU-PERSIST] Set CU metadata on ${sourceFilePath}: result #${resultNumber} -> ${cuResultBlobPath}`);
    } catch (error: any) {
      console.error(`[CU-PERSIST] Error setting CU metadata:`, error.message);
      throw error;
    }
  }
  ...
  async removeCuResultMetadata(
    ...
    try {
      ...
      for (const [key, value] of Object.entries(existingMetadata)) {
        ...
        if (key.startsWith(`cu_result_${resultNumber}_`)) {
          continue;
        }
      ...
      if (updatedNumbers.length > 0) {
        newMetadata.cu_results_list = updatedNumbers.join(',');
      } else {
        // Remove the list key if no results remain
        delete newMetadata.cu_results_list;
      }
      
      await blobClient.setMetadata(newMetadata);
      
      console.log(`[CU-PERSIST] Removed CU metadata from ${sourceFilePath}: result #${resultNumber}`);
    } catch (error: any) {
      // If source blob doesn't exist, that's OK (file was already deleted)
      if (error.statusCode === 404) {
        console.log(`[CU-PERSIST] Source blob not found during metadata removal: ${sourceFilePath}`);
        return;
      }
      console.error(`[CU-PERSIST] Error removing CU metadata:`, error.message);
      throw error;
    }
  }
  ...
  private async validateBlobOwnership(
    ...
    try {
      ...
      console.log(`[CU-PERSIST] Validating ownership for blob: ${blobPath} -> decoded: ${decodedPath}, orgId: ${organizationId}`);
      const blobClient = containerClient.getBlobClient(decodedPath);
      const exists = await blobClient.exists();
      if (!exists) {
        console.warn(`[CU-PERSIST] Blob not found: ${blobPath}`);
        return { valid: false, error: "CU result not found" };
      }

      const properties = await blobClient.getProperties();
      console.log(`[CU-PERSIST] Blob metadata keys:`, Object.keys(properties.metadata || {}));
      ...
      if (storedOrgId) {
        ...
        if (parseInt(storedOrgId, 10) !== organizationId) {
          console.warn(`[CU-PERSIST] Organization mismatch: requested ${organizationId}, stored ${storedOrgId}`);
      ...
      console.log(`[CU-PERSIST] No organizationId metadata, checking JSON content for: ${blobPath}`);
      try {
        ...
        if (jsonData.organizationId !== undefined) {
          ...
          if (contentOrgId === organizationId) {
            console.log(`[CU-PERSIST] Validated from JSON content, orgId: ${contentOrgId}`);
            return { valid: true };
          } else {
            console.warn(`[CU-PERSIST] JSON content org mismatch: requested ${organizationId}, stored ${contentOrgId}`);
            return { valid: false, error: "Access denied" };
          }
        }
      } catch (jsonError: any) {
        console.warn(`[CU-PERSIST] Failed to parse JSON content for legacy check: ${jsonError.message}`);
      }
      
      // If neither metadata nor JSON content has organizationId, deny access
      console.warn(`[CU-PERSIST] No organizationId in metadata or content for blob: ${blobPath}`);
      return { valid: false, error: "Access denied - invalid CU result" };
    } catch (error: any) {
      console.error(`[CU-PERSIST] Error validating blob ownership:`, error.message);
   ...
   */
  async saveResult(request: SaveCuResultRequest): Promise<SaveCuResultResponse> {
    const { 
      storageAccountName, 
      containerName, 
      sourceFilePath, 
    ...
    console.log(`[CU-PERSIST] Saving CU result for: ${sourceFilePath}`);
    ...
    try {
      ...
      if (untrackedResults.length > 0) {
        console.log(`[CU-PERSIST] Found ${untrackedResults.length} untracked legacy result(s), backfilling metadata`);
        for (const result of untrackedResults) {
          try {
            ...
            console.warn(`[CU-PERSIST] Failed to backfill metadata for result #${result.resultNumber}:`, backfillErr.message);
          }
        }
      }
      ...
      // Use human-readable naming: {filename}_cu_result_{timestamp}_{number}.json
      ...
      console.log(`[CU-PERSIST] DEBUG: Saving blob with metadata:`, JSON.stringify(blobMetadata));
      console.log(`[CU-PERSIST] DEBUG: organizationId=${organizationId}, userEmail=${userEmail}`);
      console.log(`[CU-PERSIST] DEBUG: blobPath=${blobPath}`);
      ...
      try {
        ...
        console.log(`[CU-PERSIST] DEBUG: Verified CU result blob metadata:`, JSON.stringify(props.metadata || {}));
      } catch (verifyErr: any) {
        console.warn(`[CU-PERSIST] DEBUG: Could not verify CU result blob metadata:`, verifyErr.message);
      }
      ...
      try {
        ...
        console.log(`[CU-PERSIST] Updated source blob metadata with CU result reference`);
      } catch (metadataErr: any) {
        // Log error but don't fail - the CU result file was saved successfully
        console.error(`[CU-PERSIST] WARNING: Failed to update source blob metadata:`, metadataErr.message);
        console.error(`[CU-PERSIST] CU result saved at ${blobPath} but metadata link may be missing`);
      }

      console.log(`[CU-PERSIST] Saved CU result: ${blobPath}`);
      ...
      console.error(`[CU-PERSIST] Error saving CU result:`, error.message);
      return {
        success: false,
   ...
   */
  private async listAllResultsInFolder(containerClient: ContainerClient, cuFolderPath: string): Promise<CuResultItem[]> {
    const results: CuResultItem[] = [];
    const prefix = `${cuFolderPath}/`;

    console.log(`[CU-PERSIST] DEBUG: Listing blobs with prefix: ${prefix}, includeMetadata: true`);

    // Include metadata in the list response
    for await (const blob of containerClient.listBlobsFlat({ prefix, includeMetadata: true })) {
      if (blob.name.endsWith('.json') && blob.name.includes(CU_RESULT_PREFIX)) {
        const blobName = blob.name.split('/').pop() || '';
        console.log(`[CU-PERSIST] DEBUG: Found blob: ${blobName}, metadata:`, JSON.stringify(blob.metadata || {}));
        results.push({
          blobPath: blob.name,
          blobName,
    ...
    console.log(`[CU-PERSIST] DEBUG: Found ${results.length} CU result(s) in folder`);
   ...
   */
  private async listResultsInternal(containerClient: ContainerClient, cuFolderPath: string, sourceFilePath: string): Promise<CuResultItem[]> {
  ...
  async listResults(
    ...
    containerName: string, 
    sourceFilePath: string
  ): Promise<ListCuResultsResponse> {
    console.log(`[CU-PERSIST] Listing CU results for: ${sourceFilePath}`);
    ...
    try {
      ...
      const validResultsFromMetadata: CuResultItem[] = [];
      
      if (cuMetadata.resultNumbers.length > 0) {
        // Convert metadata entries to CuResultItem format
        ...
        for (const [numStr, entry] of Object.entries(cuMetadata.results)) {
          ...
          try {
            ...
            if (exists) {
              ...
              console.warn(`[CU-PERSIST] CU result blob not found (stale metadata): ${entry.blobPath}`);
              staleResultNumbers.push(num);
              continue;
            }
          } catch (propErr: any) {
            console.warn(`[CU-PERSIST] Failed to get properties for ${entry.blobPath}:`, propErr.message);
          }
        ...
        if (staleResultNumbers.length > 0) {
          console.log(`[CU-PERSIST] Cleaning up ${staleResultNumbers.length} stale metadata entries`);
          for (const staleNum of staleResultNumbers) {
            try {
              ...
              console.warn(`[CU-PERSIST] Failed to clean up stale metadata for result #${staleNum}:`, cleanupErr.message);
            }
          }
        }
      }
      ...
      if (untracked.length > 0) {
        console.log(`[CU-PERSIST] Found ${untracked.length} untracked legacy result(s), adding to results and backfilling metadata`);
      ...
      console.log(`[CU-PERSIST] Found ${validResultsFromMetadata.length} CU result(s) total (metadata: ${cuMetadata.resultNumbers.length - staleResultNumbers.length}, legacy: ${untracked.length})`);
      ...
      console.error(`[CU-PERSIST] Error listing CU results:`, error.message);
      return {
        success: false,
  ...
  async getResult(
    ...
    blobPath: string,
    organizationId: number
  ): Promise<GetCuResultResponse> {
    console.log(`[CU-PERSIST] Getting CU result: ${blobPath}`);
    ...
    try {
      ...
      console.error(`[CU-PERSIST] Error getting CU result:`, error.message);
      return {
        success: false,
  ...
  async deleteResult(
    ...
    blobPath: string,
    organizationId: number
  ): Promise<DeleteCuResultResponse> {
    console.log(`[CU-PERSIST] Deleting CU result: ${blobPath}`);
    ...
    try {
      ...
      try {
        ...
        if (!sourceFilePath || !resultNumber) {
          console.log(`[CU-PERSIST] Metadata incomplete, reading JSON content for source info`);
          const downloadResponse = await blobClient.download();
          const content = await this.streamToString(downloadResponse.readableStreamBody!);
          const jsonData = JSON.parse(content);
        ...
        console.warn(`[CU-PERSIST] Could not read CU result for metadata cleanup:`, readErr.message);
      }
      
      // Delete the CU result blob
      await blobClient.deleteIfExists();
      console.log(`[CU-PERSIST] Deleted CU result blob: ${blobPath}`);
      ...
      if (sourceFilePath && resultNumber) {
        try {
          ...
          console.log(`[CU-PERSIST] Removed CU result reference from source blob metadata`);
        } catch (metadataErr: any) {
          // Log but don't fail - the CU result file was deleted successfully
          console.warn(`[CU-PERSIST] Failed to remove metadata reference:`, metadataErr.message);
        }
      } else {
        console.warn(`[CU-PERSIST] Could not determine source file path or result number for metadata cleanup`);
      }
      ...
      console.error(`[CU-PERSIST] Error deleting CU result:`, error.message);
      return {
        success: false,
  ...
  async deleteAllResultsForFile(
    ...
    console.log(`[CU-PERSIST] Cascade deleting all CU results for: ${sourceFilePath}`);
    ...
    try {
      ...
      if (cuMetadata.resultNumbers.length > 0) {
        ...
        console.log(`[CU-PERSIST] Found ${blobPathsToDelete.length} CU result(s) via metadata for cascade delete`);
      } else {
        // Fallback: Legacy folder scan
        console.log(`[CU-PERSIST] No metadata found, using legacy folder scan for cascade delete`);
        const legacyResults = await this.listResultsInternal(containerClient, cuFolderPath, sourceFilePath);
        blobPathsToDelete = legacyResults.map(r => r.blobPath);
      }
      
      if (blobPathsToDelete.length === 0) {
        console.log(`[CU-PERSIST] No CU results found for cascade delete`);
        return {
          success: true,
      ...
      for (const blobPath of blobPathsToDelete) {
        try {
          ...
          console.warn(`[CU-PERSIST] Failed to delete ${blobPath}:`, deleteError.message);
        }
      }

      console.log(`[CU-PERSIST] Cascade deleted ${deletedCount}/${blobPathsToDelete.length} CU result(s)`);
      ...
      console.error(`[CU-PERSIST] Error cascade deleting CU results:`, error.message);
      return {
        success: false,
  ...
  async generateSasUrl(
    ...
    try {
      ...
      // Decode path before Azure SDK call - SDK does its own encoding
      const decodedPath = decodeBlobPath(blobPath);
      console.log(`[CU-PERSIST] Generating SAS URL for: ${blobPath} -> decoded: ${decodedPath}`);
      const blobClient = containerClient.getBlobClient(decodedPath);
      ...
      console.error(`[CU-PERSIST] Error generating SAS URL:`, error.message);
      return {
        success: false,
   ...
   * Path: cu_folder/{directory}/{filename}_postcall_analysis.json
  ...
  async savePostCallAnalysis(
    ...
    try {
      ...
      try {
        ...
        console.warn(`[CU-PERSIST] Could not update source blob metadata with postcall ref:`, metaErr.message);
      }

      console.log(`[CU-PERSIST] Saved post-call analysis: ${blobPath}`);
      return { success: true, blobPath };
    } catch (error: any) {
      console.error(`[CU-PERSIST] Error saving post-call analysis:`, error.message);
  ...
  async getPostCallAnalysis(
    ...
    try {
      ...
      if (parsed.organizationId !== undefined && parsed.organizationId !== organizationId) {
        console.warn(`[CU-PERSIST] Post-call analysis org mismatch for: ${blobPath}`);
      ...
      console.error(`[CU-PERSIST] Error retrieving post-call analysis:`, error.message);
      return { success: false, error: error.message };
    }
  }
  ...
  // Stored at: cu_folder/{dir}/{filename}_eval_ocr_cache.json
...
export const cuPersistenceService = new CuPersistenceService();
